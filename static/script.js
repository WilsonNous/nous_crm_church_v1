...
// ✅ apiRequest mais robusto: status + msg real (json ou texto)
async function apiRequest(endpoint, method = "GET", body = null) {
  const headers = getAuthHeaders(true);
  const opts = { method, headers };
  if (body) opts.body = JSON.stringify(body);

  // permite endpoint "send-message-manual" ou "/api/send-message-manual"
  const clean = String(endpoint || "").replace(/^\/+/, "");
  const url = clean.startsWith("api/")
    ? `${baseUrl}/${clean}`
    : `${baseUrl}/api/${clean}`;
...
function collectFormData() {
  ...
  return {
    ...
    contactTime:
      document.querySelector('input[name="contactTime"]')?.value ||
      $("contactTime")?.value ||
      "",
  };
}
...
function registerVisitor(visitorData) {
  apiRequest("register", "POST", visitorData)
    ...
    .catch((err) =>
      showError(`Erro ao registrar: ${err.message}`, "registerErrorContainer")
    );
}
...
// ✅ normalização para Z-API (55 + DDD + 9XXXXXXXX)
function onlyDigits(s) {
  return String(s || "").replace(/\D/g, "");
}
function normalizePhoneForZapi(phone) {
  const n = onlyDigits(phone);
  if (!n) return null;
  if (n.startsWith("55")) return n;
  if (n.length === 10 || n.length === 11) return "55" + n;
  return n; // fallback (pra log)
}
...
function fetchVisitorsAndSendMessagesManual() {
  apiRequest("visitantes/fase-null")
    .then((data) => {
      ...
      const messages = novos
        .map((v) => {
          const numeroZapi = normalizePhoneForZapi(v.telefone);
          if (!numeroZapi) {
            appendLogToWhatsapp(`⚠️ Visitante sem telefone válido: ${v.nome || "(sem nome)"}`, true);
            return null;
          }
...
🙏 Me diga sua escolha para podermos continuar!`,
      ...
      appendLogToWhatsapp(`📌 Total para envio: ${messages.length}`);
...
async function sendMessagesSequentially(messages, delayMs = 2000) {
  for (const v of messages || []) {
    try {
      ...
      const resp = await apiRequest("send-message-manual", "POST", payload);

      // se backend responder {success:false}
      ...
      if (String(err?.status) === "400") {
        appendLogToWhatsapp(`ℹ️ (400) Erro não repetível. Pulando para o próximo.`, true);
      }
...
function loadPendingQuestions() {
  fetch(`${baseUrl}/api/ia/pending-questions`, {
    headers: getAuthHeaders(true),
    credentials: "include",
  })
    .then((r) => r.json())
...
function showTeachForm(index) {
  ...
  const questionEl = $("teachQuestion"),
    answerEl = $("teachAnswer"),
    categoryEl = $("teachCategory");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isLegacySPA) {
    const map = {
      ...
      showCampaignButton: () => {
        appState.currentView = "eventos";
        updateUI();
      },

      backToOptionsCadastroButton: () => {
        appState.currentView = "options";
        updateUI();
      },
      backToOptionsCadastroMembro: () => {
        appState.currentView = "options";
        updateUI();
      },
      backToOptionsCadastroAcolhido: () => {
        appState.currentView = "options";
        updateUI();
      },
      backToOptionsWhatsappButton: () => {
        appState.currentView = "options";
        updateUI();
      },
      ...
      backToOptionsIAButton: () => {
        appState.currentView = "options";
        updateUI();
      },
      cancelTeachButton: () => {
        toggleTeachForm(false);
        safeShow("trainingList");
      },
      backToOptionsEvento: () => {
        appState.currentView = "options";
        updateUI();
...
function handleMemberFormSubmission(e) {
  ...
  const discipuladosSelecionados = [...document.querySelectorAll("input[name='discipulados']:checked")].map(
    (el) => el.value
  );
  data["discipulados"] = discipuladosSelecionados;

  const ministeriosSelecionados = [...document.querySelectorAll("input[name='ministerios']:checked")].map(
    (el) => el.value
  );
  data["ministerios"] = ministeriosSelecionados;

  const outros = $("ministerios_outros")?.value;
  if (outros && outros.trim() !== "") data["ministerios_outros"] = outros.trim();
