...
async function filtrarVisitantes(event) {
  ...
  try {
    const resp = await fetch(`${baseUrl}/api/visitantes/filtro`, {
      method: 'POST',
      headers: {
...
async function enviarCampanha(event) {
  ...
  try {
    const resp = await fetch(`${baseUrl}/api/campanhas/enviar`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('jwt_token')}`
...
async function reprocessarFalhas() {
  ...
  try {
    const resp = await fetch(`${baseUrl}/api/campanhas/reprocessar`, {
      method: 'POST',
...
async function carregarStatus() {
  try {
    const resp = await fetch(`${baseUrl}/api/campanhas/status`, {
      headers: { 'Authorization': `Bearer ${localStorage.getItem('jwt_token')}` }
    });
    const data = await resp.json();
